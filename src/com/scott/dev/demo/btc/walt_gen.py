...
def get_address():
    ...
    while not valid_private_key:
        ...
        '''
        logger.info('decimal_encoded_public_key:{}'.format(decimal_enc_pub))
        logger.info('bin_encoded_public_key:{}'.format(bin_enc_public_key))
        logger.info('bin_compressed_encoded_public_key:{}'.format(bin_compress_encoded_pub))
        # logger.info('hex_compressed_encoded_public_key:{}'.format(hex_compress_encoded_pub))
        logger.info('bin_electrum_encoded_public_key:{}'.format(bin_electrum_enc_pub))
        logger.info('hex_electrum_encoded_public_key:{}'.format(hex_electrum_enc_pub))
        '''
        # 压缩公钥，根据y是偶数还是奇数调整前缀
