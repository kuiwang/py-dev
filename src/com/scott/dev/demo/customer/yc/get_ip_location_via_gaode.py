...
IP_API = "https://restapi.amap.com/v3/ip?key=5743f426280ea35517c2a2d91bbd3b87&ip={}"
...
def getIPList():
    ...
    ipLst = []
    select_conv_sql = 'select distinct ip from conv_info t '
    select_send_sql='select distinct t.client_ip  from send_info_new t '
    try:
        res1 = conn.queryall(select_conv_sql)
        for row in res1:
            ipLst.append(row[0])
        res2 = conn.queryall(select_send_sql)
        for row in res2:
            ipLst.append(row[0])
        return ipLst
    except Exception as e:
...
def isIPExist(ip):
    ...
    select_sql = 'select distinct ip from ip_info_gd t where t.ip= "' + ip + '" '
    try:
...
def saveIP2DB(ip, ipObj):
    ...
    param = []
    status = str(ipObj['status'])
    if status == '1':
        province = ipObj['province']
        city = ipObj['city']
        

        insert_sql = 'insert into ip_info_gd values(%s,%s,%s)'
        param.append([str(ip), str(province), str(city)])
